            ...
            <div className="panel">
                {
                    entries.map((entry, i) => (
                        <div
                            ...
                            className="entry"
                            style={{
                                backgroundPositionX: `${100 - GetPercentage(entry.votes, AppConfig.GetBool('barRelativeTop') ? topVotes : totalVotes)}%`,
                                transform: `translateY(${(entry.sortIndex - i) * 4.5}rem)`
                            }}
                        >
                            <span className="title">{entry.gameName}</span>
                            <span className="count">{GetEntryCount(entry.votes, totalVotes)}</span>
                        </div>
                    ))
                }
            </div>
